...
#define TAG "de.appwerft.signinwith"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Tisigninwith_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tisigninwith.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TisigninwithBindings::lookupGeneratedInit(
...
static void Tisigninwith_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tisigninwith_getBinding)->GetFunction());
}

static void Tisigninwith_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TisigninwithBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TisigninwithBinding = {
	"de.appwerft.signinwith",
	Tisigninwith_init,
	Tisigninwith_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_de_appwerft_signinwith_TisigninwithBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.signinwith", &TisigninwithBinding);
	titanium::KrollBindings::addExternalLookup(&(::TisigninwithBindings::lookupGeneratedInit));
}
