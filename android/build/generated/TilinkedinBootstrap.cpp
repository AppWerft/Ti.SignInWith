...
#define TAG "de.appwerft.linkedin"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Tilinkedin_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tilinkedin.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TilinkedinBindings::lookupGeneratedInit(
...
static void Tilinkedin_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tilinkedin_getBinding)->GetFunction());
}

static void Tilinkedin_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TilinkedinBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TilinkedinBinding = {
	"de.appwerft.linkedin",
	Tilinkedin_init,
	Tilinkedin_dispose
};
...
Java_de_appwerft_linkedin_TilinkedinBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.linkedin", &TilinkedinBinding);
	titanium::KrollBindings::addExternalLookup(&(::TilinkedinBindings::lookupGeneratedInit));
}
